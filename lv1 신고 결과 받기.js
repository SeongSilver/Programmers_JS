...
function solution(id_list, report, k) {
    ...
    for(let i = 0; i<reportFiltered.length;i++){
        ...
        // 신고된 아이디가 있는 id_list의 index를 변수에 초기화
        let idxOfReported = id_list.indexOf(reportedId1);
        //id_list와 reportCount는 순서가 같으므로 해당 인덱스에 값 1추가
        reportCount[idxOfReported]+=1;
        //reportCount가 k보다 크거나 같으면 정지된 아이디에 id_list에서 신고된 아이디 찾아서 push
    ...
    reportFiltered.map(data => {
        ...
        //정지된 아이디 배열에서 신고된 아이디 찾아서 그 값이 있을 때
        if(stopedId.includes(reportedId2)){
            //신고된 아이디와 같은 값이 있는 구조분해 할당 받은 신고한 아이디를 indexOf로 id_list에서 위치를 찾아서 변수에초기화
            let existReportIdx = id_list.indexOf(reportId2);
            //answer과 id_list의 순서는 같기 떄문에 해당 인덱스에 값추가
